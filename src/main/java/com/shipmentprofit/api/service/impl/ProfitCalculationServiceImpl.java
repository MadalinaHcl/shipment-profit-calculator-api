...
/**
 * Recalculating a shipment does not append duplicate financial records.
 * A shipment has at most one income record, one BASE cost and one
 * ADDITIONAL cost. Existing values are updated in place, so repeated
 * submissions with the same financial input produce the same totals.
 */
@Service
public class ProfitCalculationServiceImpl implements ProfitCalculationService {
    ...
    public ProfitCalculationServiceImpl(ShipmentRepository shipmentRepository,
    ...
    public ProfitLossResponse calculateProfit(CalculateProfitRequest request) {
        ...
        Optional<Income> existingIncome = resolution.isNew()
                ? Optional.empty()
                : incomeRepository.findByShipmentId(shipment.getId());
        List<Cost> existingCosts = resolution.isNew()
                ? List.of()
                : costRepository.findByShipmentId(shipment.getId());
        ...
        BigDecimal totalCosts =
                baseCost.getAmount().add(additionalCost);
        BigDecimal profitOrLoss =
                totalIncome.subtract(totalCosts);

        Optional<ProfitLoss> existingResult = resolution.isNew()
                ? Optional.empty()
                : profitLossRepository.findByShipmentId(shipment.getId());
        ProfitLoss result = saveOrUpdateResult(
                shipment,
                totalIncome,
                totalCosts,
                profitOrLoss,
        ...
        return new ProfitLossResponse(shipment.getShipmentCode(), totalIncome, totalCosts, profitOrLoss,
    ...
    private Income saveOrUpdateIncome(Shipment shipment, BigDecimal amount, Optional<Income> existing) {
        Income income = findOrCreate(
                existing,
    ...
    /**
     * Updates the shipment's ADDITIONAL cost to match the request, deleting it
     * when the request no longer carries one. Returns the amount to add to the
     * total (zero when there is no ADDITIONAL cost).
     */
    private BigDecimal synchronizeAdditionalCost(Shipment shipment, BigDecimal amount, Optional<Cost> existing) {
        if (amount.signum() == 0) {
            existing.ifPresent(costRepository::delete);
            return BigDecimal.ZERO;
        }

        Cost additionalCost = findOrCreate(
                existing,
                current -> current.updateAmount(amount),
    ...
    private ProfitLoss saveOrUpdateResult(Shipment shipment, BigDecimal totalIncome, BigDecimal totalCosts,
                                           BigDecimal profitOrLoss, Optional<ProfitLoss> existing) {
        ProfitLoss profitLoss = findOrCreate(
                existing,

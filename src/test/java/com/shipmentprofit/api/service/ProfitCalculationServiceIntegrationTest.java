...
/**
 * Recalculating the same shipment must be idempotent: no duplicate
 * income/cost rows, and each recalculation reflects only the latest
 * submitted values - never the sum of everything ever submitted.
 */
@SpringBootTest
class ProfitCalculationServiceIntegrationTest {
    ...
    void sameRequestTwiceProducesSameTotalsAndNoDuplicateRows() {
        CalculateProfitRequest request = new CalculateProfitRequest("IT-SAME-TWICE", new BigDecimal("230.50"),
        ...
        assertThat(costRepository.findByShipmentId(shipment.getId()))
                ...
                .hasSize(1);
        assertThat(costRepository.findByShipmentIdAndCostType(shipment.getId(), CostType.BASE)).isPresent();
    }

    @Test
    void incomeAndBaseCostAreReplacedNotAccumulatedOnRecalculation() {
        CalculateProfitRequest firstRequest = new CalculateProfitRequest("IT-INCOME-COST-REPLACE", new BigDecimal("1000.00"),
                new BigDecimal("200.00"), BigDecimal.ZERO);
        CalculateProfitRequest secondRequest = new CalculateProfitRequest("IT-INCOME-COST-REPLACE", new BigDecimal("800.00"),
        ...
        Shipment shipment = shipmentRepository.findByShipmentCode("IT-INCOME-COST-REPLACE").orElseThrow();
        assertThat(incomeRepository.findByShipmentId(shipment.getId()))
                .get().extracting(i -> i.getAmount())
    ...
    void secondAdditionalCostReplacesTheFirstRatherThanAddingToIt() {
        CalculateProfitRequest fiftyAdditional = new CalculateProfitRequest("IT-ADD-REPLACE", new BigDecimal("1000.00"),
                new BigDecimal("200.00"), new BigDecimal("50.00"));
        CalculateProfitRequest twentyAdditional = new CalculateProfitRequest("IT-ADD-REPLACE", new BigDecimal("1000.00"),
        ...
        assertThat(costRepository.findByShipmentIdAndCostType(shipment.getId(), CostType.ADDITIONAL))
                .get().extracting(c -> c.getAmount())
                .isEqualTo(new BigDecimal("20.00"));
    }

    @Test
    void clearingAdditionalCostToZeroRemovesItFromTheTotal() {
        CalculateProfitRequest withAdditional = new CalculateProfitRequest("IT-ADD-CLEAR", new BigDecimal("1000.00"),
                new BigDecimal("200.00"), new BigDecimal("50.00"));
        CalculateProfitRequest withoutAdditional = new CalculateProfitRequest("IT-ADD-CLEAR", new BigDecimal("1000.00"),
                new BigDecimal("200.00"), BigDecimal.ZERO);

...
class ProfitCalculationServiceImplTest {
  ...
  @Mock private ProfitLossRepository profitLossRepository;
  @Mock private ProfitLossMapper profitLossMapper;

  @InjectMocks private ProfitCalculationServiceImpl profitCalculationService;
  ...
  void setUp() {
    ...
    when(profitLossMapper.toResponse(any(ProfitLoss.class)))
        .thenAnswer(
            invocation -> {
              ProfitLoss pl = invocation.getArgument(0);
              return new ProfitLossResponse(
                  pl.getShipment().getShipmentCode(),
                  pl.getTotalIncome(),
                  pl.getTotalCosts(),
                  pl.getProfitOrLoss(),
                  pl.getCalculatedAt());
  ...
  void calculatesProfitWhenIncomeExceedsCosts() {
    ...
    when(profitLossRepository.save(any(ProfitLoss.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
  ...
  void calculatesLossWhenCostsExceedIncome() {
    CalculateProfitRequest request =
        new CalculateProfitRequest(
            "0002", new BigDecimal("500.00"), new BigDecimal("900.00"), BigDecimal.ZERO);
  ...
  void createsNewShipmentWhenShipmentCodeNotFound() {
    CalculateProfitRequest request =
        new CalculateProfitRequest(
            "9999", new BigDecimal("100.00"), new BigDecimal("50.00"), BigDecimal.ZERO);
  ...
  void recalculatingUpdatesExistingIncomeAndCostInsteadOfInsertingNewOnes() {
    CalculateProfitRequest request =
        new CalculateProfitRequest(
            "0001", new BigDecimal("230.50"), new BigDecimal("200.00"), BigDecimal.ZERO);
    Income existingIncome = new Income(shipment, new BigDecimal("1000.00"), "Customer payment");
    Cost existingBaseCost =
        new Cost(shipment, CostType.BASE, new BigDecimal("200.00"), "Service provision cost");

    when(shipmentRepository.findByShipmentCode("0001")).thenReturn(Optional.of(shipment));
    when(incomeRepository.findByShipmentId(shipment.getId()))
    ...
    // the same Income/Cost instances were updated and saved, never a fresh "new Income(...)"/"new
    // Cost(...)"
  ...
  void clearingAdditionalCostDeletesThePreviouslyStoredRow() {
    ...
    Cost existingBaseCost =
        new Cost(shipment, CostType.BASE, new BigDecimal("200.00"), "Service provision cost");
    Cost existingAdditionalCost =
        new Cost(shipment, CostType.ADDITIONAL, new BigDecimal("50.00"), "Additional cost");
    ...
    when(incomeRepository.save(any(Income.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
    when(costRepository.findByShipmentId(shipment.getId()))
        .thenReturn(List.of(existingBaseCost, existingAdditionalCost));
    when(costRepository.save(any(Cost.class))).thenAnswer(invocation -> invocation.getArgument(0));
    when(profitLossRepository.findByShipmentId(shipment.getId())).thenReturn(Optional.empty());
    when(profitLossRepository.save(any(ProfitLoss.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));

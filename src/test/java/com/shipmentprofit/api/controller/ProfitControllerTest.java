...
class ProfitControllerTest {
    ...
    void returnsCalculatedProfitForValidRequest() throws Exception {
        ...
        ProfitLossResponse response = new ProfitLossResponse("0001", new BigDecimal("1000.00"),
                new BigDecimal("200.00"), new BigDecimal("800.00"), LocalDateTime.now());

        when(profitCalculationService.calculateProfit(any(CalculateProfitRequest.class))).thenReturn(response);

        mockMvc.perform(post("/api/profit-calculations")
    ...
    void returnsBadRequestWhenShipmentCodeMissing() throws Exception {
        CalculateProfitRequest request = new CalculateProfitRequest("", new BigDecimal("1000.00"),
                new BigDecimal("200.00"), BigDecimal.ZERO);
